...
export function FloatingWhatsApp() {
  ...
  return (
    <EnrollButton
      aria-label={t("label")}
      className={cn(
        "group fixed z-40 flex items-center gap-0 overflow-hidden rounded-full",
        "bg-brand text-primary-foreground shadow-[0_4px_20px_rgba(33,56,184,0.28)]",
        "transition-all duration-300 ease-out",
        "hover:bg-brand-light hover:shadow-[0_6px_28px_rgba(56,102,242,0.4)]",
        ...
        "h-12 w-12 sm:bottom-6 sm:right-6 sm:h-14 sm:w-14",
        "sm:hover:w-auto sm:hover:gap-2.5 sm:hover:px-4",
      )}
    >
      ...
      <span
        className={cn(
          ...
          "transition-all duration-300 sm:group-hover:mr-1 sm:group-hover:inline sm:group-hover:max-w-[10rem] sm:group-hover:opacity-100",
        )}
      >
        {t("shortLabel")}
      </span>
